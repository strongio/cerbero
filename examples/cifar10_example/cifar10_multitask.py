...
(
    ...
    (rgb_images_train, rgb_labels_train),
) = make_cifar_dataset(train=True)
...
(
    ...
    class_labels_train,
    class_labels_test,
) = train_test_split(class_images_train, class_labels_train, test_size=0.05)
...
(
    ...
    rgb_labels_train,
    rgb_labels_test,
) = train_test_split(rgb_images_train, rgb_labels_train, test_size=0.1)

(
    (class_images_val, class_labels_val),
    (rgb_images_val, rgb_labels_val),
) = make_cifar_dataset(train=False)
# -
...
op1 = Operation(
    name="base_net", module_name="base_net", inputs=[("_input_", "class_data")]
)

# Op2: pass output of Op1 to the class head module
op2 = Operation(name="class_head", module_name="class_head_module", inputs=["base_net"])
...
op_sequence = [
    Operation(
        name="base_net", module_name="base_net", inputs=[("_input_", "rgb_data")]
    ),
    Operation(name="rgb_head", module_name="rgb_head_module", inputs=["base_net"]),
]
...
rgb_task = Task(
    name="rgb_task",
    module_pool=nn.ModuleDict(
        {"base_net": base_net, "rgb_head_module": rgb_head_module}
    ),
    op_sequence=op_sequence,
...
norm_images = torch.stack(
    [norm_transform(torch.Tensor(_image)) for _image in torch_images], dim=0
)
# -

input_dict = {"class_data": norm_images, "rgb_data": norm_images}
with torch.no_grad():
    out_dict = model(input_dict, task_names=["class_task", "rgb_task"])

class_out = torch.argmax(F.softmax(out_dict["class_head"], dim=1), dim=1)
